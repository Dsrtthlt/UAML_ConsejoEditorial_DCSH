...
\urlstyle{same}
\hypersetup{
  pdftitle={Hegemonía algorítmica y visibilidad académica. Una propuesta metodológica},
  pdfauthor={Christian Hernández Pérez},
  pdfkeywords={Comunicación académica, Algoritmos, Visibilidad},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{Hegemonía algorítmica y visibilidad académica. Una propuesta
metodológica}
\usepackage{etoolbox}
\makeatletter
...
\makeatother
\subtitle{Algorithmic Hegemony and Academic Visibility: A Methodological
Proposal}
\author{Christian Hernández Pérez \textbar\ \textbar\ 
Universidad Autónoma Metropolitana, Unidad Azcapotzalco}


\thanks{CONTACTO: \href{mailto:yadgana@gmail.com}{yadgana@gmail.com}.}
\begin{document}
...
Comunicación académica, Algoritmos, Visibilidad
\end{footnotesize}
...
Academic communication, Algorithms, Visibility
\end{footnotesize}
...
social (Raine and Wellman 2012), ha permeado a la sociedad a tal grado
...
que no son públicos y mucho menos imparciales (Halavais 2009: 15). Estas
máquinas de cálculo arrojan una serie de resultados ``objetivos'' que
los usuarios supuestamente ``eligen'' para satisfacer sus necesidades de
información.
...
matemáticos y supuestamente ``meritocráticos'' (Cardon 2013: 63).
...
extraer\footnote{Esta propuesta no se detiene a buscar elementos para
  descajanegrizar la manera en la que operan los algoritmos de búsqueda.
  Solo está centrado en utilizarlos como una herramienta de recuperación
  de información y mostrar cómo este algoritmo puede influir
  significativamente en la percepción que las personas, ajenas al mundo
  académico, pueden llegar a formarse del trabajo de los investigadores.}.
...
\item
  ...
  información\footnote{Según Statista, una empresa de analítica web que
    ...
    predilecto (Halavais 2009: 6).}. De ahí que la información que
  muestra el algoritmo de búsqueda tiene una influencia significativa en
  la percepción que las personas pueden formarse de un tema en
  particular.
\end{enumerate}
...
académicos adscriptos al Sistema Nacional de Investigadores (SNI). La
...
importancia de la difusión en su trabajo, perfil público y homogéneo.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Difusión de resultados. Debido a que uno de los requisitos para
  pertenecer al SNI radica en que los miembros publiquen resultados de
  ...
  repositorios institucionales). Esta producción
\item
  ...
  padrón del SNI permite reunir perfiles académicos en mayor medida
...
Investigadores que permanecieron vigentes durante el periodo de
2010-2017. Esta selección generacional ha permitido registrar el perfil
de investigadores que se encontraban en diferentes estadios de su
trayectoria académica (Candidatos, Niveles I, II y III) y que pertenecen
a diferentes campos de estudio, el de la Física (n=50) y la Sociología
(n=31)\footnote{La fuente de datos original del Sistema Nacional de
  Investigadores, la que incluye a los miembros de la física y la
  sociología (N=3465) durante el periodo antes señalado, fue
  proporcionada por la Red de Observatorios sobre la Movilidad Académica
  y Científica de mexicanos (ROMAC)
...
probabilidad antes mencionados: Alta, Media y Baja. De tal modo que los
sitios web que aparecen en la primera y segunda posiciones presentarían
una Alta visibilidad debido a que en el 58.5\% de las veces que los
usuarios realizan una búsqueda, en este caso con el nombre propio del
académico, estos accederían al contenido del destino web que se
encuentra en dichas posiciones. Por contrario, el contenido que se
exponga en alguno de los sitios web ubicados entre las posiciones 6-10
tendría una visibilidad Baja o marginal de consulta en el 13\% de los
casos (ver la Tabla 1).

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3056}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2361}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2778}}@{}}
\caption{\emph{Indicadores de visibilidad}}\tabularnewline
\toprule\noalign{}
\endfirsthead
\endhead
\bottomrule\noalign{}
...
presentaron los grados Alto y Bajo de visibilidad.\footnote{De igual
  ...
  cuadro de Tipologías de sitios web.} Así pues, esta propuesta describe
...
\item
  ...
  formales fueron lanzados con la intención de que almacenar, organizar
  y difundir las publicaciones de los investigadores. Paradójicamente
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{images/clipboard-166497021.png}}

}

\caption{Representación espacial de los indicadores de visibilidad y la
...
e incluso en la reputación de una persona. Durante esta observación
pudieron ubicarse un par de casos polémicos que ameritan mención y que
ponen de relieve el papel de los algoritmos de búsqueda en visibilizar u
ocultar determinado tema.

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{images/clipboard-1979658417.png}}

}

\caption{Caso 1. Al escribir el nombre de este académico sobre la caja
de búsqueda, Google arroja resultados de autocompletado, una predicción
de lo que probablemente el usuario quiere encontrar sobre este
particular término. El autocompletado muestra las tendencias o consultas
recurrentes que han hecho los usuarios y que están relacionadas con el
término de búsqueda. Esta captura de pantalla presenta el nombre del
académico (borrado por cuestiones de privacidad) seguido de una palabra
que refleja las principales consultas de búsqueda. En este ejemplo
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{images/clipboard-1919401924.png}}

}

\caption{Caso 2. Captura de pantalla de un autocompletado de búsqueda
que involucra a un académico en un supuesto caso de acoso. Entre los
primeros resultados del autocompletado se muestra la palabra
``acosador''. En la actualidad (febrero de 2025) este resultado de
autocompletado ya no existe. Cabe recordar que los resultados de
búsqueda cambian constantemente ante la actualización periódica del
algoritmo y el constante incremento y cambio en el volumen de
información que éste analiza.}
...
\caption{\emph{Tipologías de sitios web. Ejemplo de las tipologías con
mayor frecuencia en la clasificación de los sitios web. Para fines de
esta propuesta solo se muestran los casos contrastantes que se mencionan
en el apartado de ``Tipología y grados de
visibilidad''.}}\tabularnewline
\toprule\noalign{}
\endfirsthead
\endhead
\bottomrule\noalign{}
...
\end{minipage} & \begin{minipage}[t]{\linewidth}\raggedright
\begin{quote}
Sitio donde está adscripto el académico como docente o realizando
...
\end{minipage} & \begin{minipage}[t]{\linewidth}\raggedright
\begin{quote}
El titular de la cuenta es el investigador
\end{quote}
\end{minipage} & \begin{minipage}[t]{\linewidth}\raggedright
\begin{quote}
I\_Linkedin
...
F\_PIp
\end{quote}
\end{minipage} & 27 & \begin{minipage}[t]{\linewidth}\raggedright
\begin{quote}
29.3
\end{quote}
\end{minipage} & \begin{minipage}[t]{\linewidth}\raggedright
...
\bibitem[\citeproctext]{ref-Cardon2013}
Cardon, Dominique. 2013. {``Dans l'esprit Du {PageRank}: {Une} Enqu{ê}te
Sur l'algoritme de {Google}.''} \emph{R{é}seaux} 177: 63--95.
\url{https://doi.org/10.3917/res.177.0063}.

\bibitem[\citeproctext]{ref-Halavais2009}
Halavais, Alexander. 2009. {``The Engines.''} In \emph{Search Engine
Society}, 5--31. Cambridge: Polity Press.

\bibitem[\citeproctext]{ref-Raine2012}
Raine, L., and B. Wellman. 2012. {``The Triple Revolution.''} In
\emph{Networked. {The} New Social Operating System}, 3--108. Cambridge,
MA: MIT Press.

\bibitem[\citeproctext]{ref-Sued2022}
Sued, Gabriela Elisa. 2022. {``Culturas Algor{í}tmicas: Conceptos y
m{é}todos Para Su Estudio Social.''} \emph{Revista Mexicana de Ciencias
Pol{í}ticas y Sociales} 67 (246): 43--73.
